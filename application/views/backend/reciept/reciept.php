...
<title>Transaction - <?php echo $row['id']; ?></title>
...
<small>Client</small>
<?php
 $client_id= $this->db->get_where('account',array('id'=>$row['account_id']))->row()->client_id;
 $client_detaisl= $this->db->get_where('client',array('id'=>$client_id))->result_array();
 foreach ($client_detaisl as $rowz): ?>
<address class="m-t-5 m-b-5">
<strong class="text-inverse"><?php echo $rowz['lastname']." ".$rowz['fullname'];?></strong><br>
<?php echo $rowz['address'];?><br>
City, Zip Code<br>
Phone: <?php echo $rowz['contact']; ?><br>
Email: <?php echo $rowz['email']; ?>
</address>
<?php
endforeach;
?>
...
Transaction id #<?php echo $row['id']; ?><br>
<?php echo $row['type']; ?>
</div>
</div>
</div>
...
<div class="container">
  <table class="table">
    ...
    <tbody>
      ...
      <tr>
        <th>New Balance</th>
        <td><?php echo "E ".$this->db->get_where('account',array('id'=>$row['account_id']))->row()->balance; ?></td>
      </tr>      
      <tr class="danger">
          <th>Time</th>
      ...
      <tr class="info">
          <th>Clerk</th>
        <td><?php echo $this->db->get_where('admin' , array('admin_id' =>$row['user']))->row()->name; ?></td>
      </tr>
    </tbody>
  </table>
</div>
...
* The Intelligence in banking<br>
* Unified in banking<br>
...
window.addEventListener("pageshow", function(event) {
    if (event.persisted) {
        window.location.reload(); // Forces a fresh reload without parameters
    }
});
