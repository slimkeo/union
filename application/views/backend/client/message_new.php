	...
	<div class="panel">
		...
		<div class="compose">
			<select class="form-control" data-plugin-selectTwo data-width="100%" name="reciever" required>
				...
				<optgroup label="<?php echo get_phrase('administrator'); ?>">
					<?php
					$administrators = $this->db->get( 'administrator' )->result_array();
					foreach ( $administrators as $row ):
						?>

					<option value="administrator-<?php echo $row['administrator_id']; ?>">- <?php echo $row['name']; ?></option>
				...
				<optgroup label="<?php echo get_phrase('admin'); ?>">
					...
					<option value="admin-<?php echo $row['admin_id']; ?>">- <?php echo $row['name']; ?></option>

					<?php endforeach; ?>
				</optgroup>
				<optgroup label="<?php echo get_phrase('supervisor'); ?>">
					<?php
					$supervisors = $this->db->get( 'supervisor' )->result_array();
					foreach ( $supervisors as $row ):
						?>

					<option value="supervisor-<?php echo $row['supervisor_id']; ?>">- <?php echo $row['name']; ?></option>
